...
export const SolvingPage = () => {
  ...
  const handleBoundingBoxChange = (coordinates: { topLeft: { x: number; y: number; }; bottomRight: { x: number; y: number; }; }) => {
    setIsBounded(true)
    setResponseObj({
      id: currentImage.id,
  ...
  return (
    <div className="solving-page_wrapper">
      ...
      <BoundingBox resetTransform={resetTransform}  currentImage={currentImage} setCurrentImage={setCurrentImage} onChange={() =>handleBoundingBoxChange} />
      <div className='solving-page_buttons'>
        <Button disabled={!isBounded} className='submit_button' onClick={trottleSubnitHandler}>Submit</Button>
        <Button className="delete-box" onClick={handleDeleteBox}>Delete the box</Button>
